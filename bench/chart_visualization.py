...
def extract_benchmark_data(results: List[Dict[str, Any]]) -> Dict[str, Dict[str, List[Tuple[int, float]]]]:
    """从结果中提取基准测试数据"""
    data = {
        'insert': {'sqlite': [], 'orm': []},
        'query': {'sqlite': [], 'orm': []},
...
def create_comparison_charts(data: Dict[str, Dict[str, List[Tuple[int, float]]]], 
                           title_prefix: str = "性能对比"):
    """创建性能对比图表（柱状图+折线图组合）"""
    ...
    fig.suptitle(f'{title_prefix} - SQLite3 vs ORM', fontsize=20, fontweight='bold', y=0.98)
    
    operations = [
        ('insert', '批量插入', axes[0, 0]),
        ('query', '批量查询', axes[0, 1]),
        ('update', '批量更新', axes[1, 0]),
        ('delete', '批量删除', axes[1, 1])
    ]
    ...
    for op_key, op_name, ax in operations:
        ...
        # 绘制柱状图
        bars1 = ax.bar(x - width/2, sqlite_times_filtered, width, 
                      label='SQLite3', color='#3498db', alpha=0.8)
        bars2 = ax.bar(x + width/2, orm_times_filtered, width, 
                      label='ORM', color='#e74c3c', alpha=0.8)
        ...
        # 创建第二个Y轴用于开销百分比
        ax2 = ax.twinx()
        ax2.plot(x, overheads, 'o-', color='#2ecc71', linewidth=3, markersize=8, 
                label='ORM开销百分比', markerfacecolor='white', markeredgewidth=2)
        
        # 设置图表属性
        ax.set_title(f'{op_name}性能对比', fontsize=16, fontweight='bold', pad=20)
        ax.set_xlabel('批量大小', fontsize=14)
        ax.set_ylabel('执行时间 (ms)', fontsize=14, color='#2c3e50')
        ax2.set_ylabel('ORM开销百分比 (%)', fontsize=14, color='#2ecc71')
        ...
        for i, overhead in enumerate(overheads):
            ax2.text(x[i], overhead + 1, f'{overhead:.1f}%', 
        ...
        ax2.set_ylim(0, max_overhead * 1.3)
    
    plt.subplots_adjust(left=0.08, right=0.92, top=0.92, bottom=0.08, hspace=0.3, wspace=0.3)
    return fig
...
def create_overhead_chart(data: Dict[str, Dict[str, List[Tuple[int, float]]]]):
    ...
    # 创建柱状图
    bars = ax.bar(op_names, avg_overheads, color=['#2E86AB', '#A23B72', '#F18F01', '#C73E1D'])
    
    # 设置图表属性
    ax.set_title('ORM平均性能开销百分比', fontsize=16, fontweight='bold')
    ax.set_xlabel('操作类型', fontsize=14)
    ax.set_ylabel('开销百分比 (%)', fontsize=14)
    ax.grid(True, alpha=0.3, axis='y')
    ...
    ax.axhline(y=20, color='green', linestyle='--', linewidth=1, alpha=0.5, label='良好 (≤20%)')
    ax.axhline(y=40, color='orange', linestyle='--', linewidth=1, alpha=0.5, label='可接受 (≤40%)')
    ax.axhline(y=60, color='red', linestyle='--', linewidth=1, alpha=0.5, label='较差 (>40%)')
...
def create_scaling_chart(data: Dict[str, Dict[str, List[Tuple[int, float]]]]):
    ...
    fig, axes = plt.subplots(2, 2, figsize=(18, 14))
    fig.suptitle('每项操作时间扩展性分析 - SQLite3 vs ORM', fontsize=20, fontweight='bold', y=0.98)
    ...
    for op_key, op_name, ax in operations:
        ...
        # 绘制柱状图
        bars1 = ax.bar(x - width/2, sqlite_per_item_filtered, width, 
                      label='SQLite3', color='#3498db', alpha=0.8)
        bars2 = ax.bar(x + width/2, orm_per_item_filtered, width, 
                      label='ORM', color='#e74c3c', alpha=0.8)
        
        # 计算效率提升百分比（负值为开销）
        efficiency_changes = []
        for sqlite_time, orm_time in zip(sqlite_per_item_filtered, orm_per_item_filtered):
            if sqlite_time > 0:
        ...
        # 创建第二个Y轴用于效率变化
        ax2 = ax.twinx()
        ax2.plot(x, efficiency_changes, 'o-', color='#9b59b6', linewidth=3, markersize=8,
                label='效率变化', markerfacecolor='white', markeredgewidth=2)
        
        # 设置图表属性
        ax.set_title(f'{op_name} - 每项操作时间', fontsize=14, fontweight='bold', pad=15)
        ...
        for i, change in enumerate(efficiency_changes):
            ...
            ax2.text(x[i], change + (1 if change >= 0 else -3), f'{prefix}{change:.1f}%', 
...
def main():
    ...
    # 检查结果文件
    result_files = [
        ('build/bench/comprehensive_results.json', '内存数据库全面基准测试'),
        ('build/bench/file_results.json', '文件数据库基准测试')
    ]
    ...
    for result_file, title in result_files:
        ...
        try:
            ...
            fig1.savefig(f'{output_dir}/{filename_base}_comparison.png', dpi=300, bbox_inches='tight')
            fig2.savefig(f'{output_dir}/{filename_base}_overhead.png', dpi=300, bbox_inches='tight')
            fig3.savefig(f'{output_dir}/{filename_base}_scaling.png', dpi=300, bbox_inches='tight')
