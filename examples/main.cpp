...
int main() {
  Invalid::Schema::make_create_table_ddl();
  return 0;
}
...
void setup_test_tables() {
  ...
  conn.execute_raw("INSERT OR IGNORE INTO category VALUES(1,'electronics') ");
  conn.execute_raw("INSERT OR IGNORE INTO category VALUES(2, 'books') ");

  // 插入测试数据
  cout << "插入测试数据:" << endl;
  query<
      "INSERT INTO goods (title, price, stock, status, enabled) VALUES ('商品1', 10.5, 100, 0, 1) "_sql>();
  query<
      "INSERT INTO goods (title, price, stock, status, enabled) VALUES ('商品2', 25.0, 50, 0, 1) "_sql>();
  query<
      "INSERT INTO goods (title, price, stock, status, enabled) VALUES ('商品3', 99.9, 20, 1, 1) "_sql>();
  query<
      "INSERT INTO goods (title, price, stock, status, enabled) VALUES ('商品4', 15.0, 200, 0, 0) "_sql>();
  query<
      "INSERT INTO goods (title, price, stock, status, enabled) VALUES ('商品5', 45.5, 75, 0, 1) "_sql>();
}
...
void test_queries_with_table_type() {
  ...
  query<Goods, "SELECT * FROM (SELECT id, title FROM goods)"_sql>();
}
...
void test_table_type_restrictions() {
  ...
  // query<Goods,
  //       "SELECT * FROM goods JOIN category ON goods.cid =
  //       category.id"_sql>();
  // 编译错误：has_joins() 返回true，验证失败
...
void test_dml_operations() {
  ...
  query<Goods, "INSERT INTO goods (title, price) VALUES (?, ?)"_sql>("test2",
                                                                     20.0);

  // 11. UPDATE操作（更新刚插入的数据）
  cout << "\n11. UPDATE操作:" << endl;
  query<Goods, "UPDATE goods SET price = 15.0 WHERE title = 'test'"_sql>();
  query<Goods, "UPDATE goods SET price = ? WHERE title = ?"_sql>(25.0, "test2");
