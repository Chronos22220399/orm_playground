...
void test_row() {
  ...
  auto pool =
      core::ConnectionPool<config::dialect>::create(MainDB::connection_url, 10);
  ...
  while (stmt.next()) {
    ...
    auto price = row.price;
    auto stock = row.stock;
    ...
    std::cout << id << " " << title << " " << price << " " << stock << " "
              << (int)status << " " << enabled << std::endl;
    goods = mapper.map_row(stmt);
    std::cout << goods.id << " " << goods.title << " " << goods.price << " "
  ...
  while (stmt_1.next()) {
    auto mapper = ResultSetMapper<Goods>{};
    mapper.init_mapper(stmt_1);
    auto row = mapper.map_row(stmt_1);
